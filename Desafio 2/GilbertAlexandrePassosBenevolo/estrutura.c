...
Estrutura* cria_estrutura() {
    ...
    for (int i = 0; i < 6; i++) e->tabela_pessoas[i] = cria_lista();
    for (int i = 0; i < 2; i++) e->tabela_criancas[i] = cria_lista();
    for (int i = 0; i < 4; i++) e->tabela_renda[i] = cria_lista();
    return e;
}

void adiciona_cliente(Estrutura* e, Cliente* c) {
...
void libera_estrutura(Estrutura* e) {
    ...
    for (int i = 0; i < 6; i++) libera_lista(e->tabela_pessoas[i]);
    for (int i = 0; i < 2; i++) libera_lista(e->tabela_criancas[i]);
    for (int i = 0; i < 4; i++) libera_lista(e->tabela_renda[i]);
    free(e);
}
